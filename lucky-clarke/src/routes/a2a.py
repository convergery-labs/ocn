...
_AGENT_CARD: dict[str, Any] = {
    ...
    "name": "lucky-clarke",
    "description": (
        "Daily AI news digest agent. Retrieves and classifies AI news "
        "articles into 12 investment-research categories, generates "
        "LLM summaries, and emails a digest to configured recipients."
    ),
    "url": "http://lucky-clarke",
    "version": "1.0.0",
    "capabilities": {
        "tasks": True,
        "streaming": False,
    ...
    "skills": [
        {
            ...
            "name": "Run digest",
            "description": (
                "Check for today's classification, trigger news "
                "retrieval + classification if needed, and send the "
                "daily digest email."
            ),
            "input_modes": ["text"],
            "output_modes": ["text"],
...
def _error_response(
    rpc_id: Any, code: int, message: str
) -> JSONResponse:
